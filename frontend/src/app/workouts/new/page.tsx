...
export default function NewWorkoutPage() {
  ...
  const [exercisePickerFor, setExercisePickerFor] = useState<string | null>(null); // null=closed, "new"=adding, blockId=changing
  ...
  const handlePickExercise = (exerciseId: string) => {
    if (exercisePickerFor === "new") {
      // Create new block with selected exercise
      const id = crypto.randomUUID();
      focusBlockIdRef.current = id;
      setBlocks((prev) => [...prev, { id, exerciseId, side: "", memo: "", sets: [newSet()] }]);
      // Fetch last sets
      exercisesApi.lastSets(parseInt(exerciseId), { side: undefined }).then((sets) => {
        setLastSetsMap((prev) => ({ ...prev, [id]: sets }));
      }).catch(() => {});
    } else if (exercisePickerFor) {
      // Change exercise on existing block
  ...
  const handleExerciseChange = (blockId: string, exerciseId: string) => {
    ...
    if (exerciseId) {
      ...
      exercisesApi.lastSets(parseInt(exerciseId), { side: block?.side || undefined }).then((sets) => {
  ...
  const updateBlock = (blockId: string, field: "exerciseId" | "side" | "memo", value: string) => {
    ...
    if (field === "side") {
      const block = blocks.find((b) => b.id === blockId);
      if (block?.exerciseId) {
        exercisesApi.lastSets(parseInt(block.exerciseId), { side: value || undefined }).then((sets) => {
          setLastSetsMap((prev) => ({ ...prev, [blockId]: sets }));
        }).catch(() => {});
      }
    }
  ...
  const updateSet = (blockId: string, setId: string, field: "weight" | "reps", value: string) => {
    // weight: digits + optional decimal point, reps: digits only
